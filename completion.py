...
class CompletionExtension_base(Completion_base):
    ...
    def _eisenstein_model(self):
        ...
        EXAMPLES::
            ...
            sage: C12 = C.extension(f.change_ring(C).change_variable_name('t'))
            sage: C12._eisenstein_model()

        """
        if self.gen().valuation() == self.valuation().value_group().gen():
            return self._base, self._base_valuation

        generator = self.valuation().lift(self.valuation().residue_field().gen()) * self.uniformizer()
        charpoly = generator.matrix(self.base_ring()).charpoly()
        error = self.base_ring()._krasner_bound(charpoly) + self.value_group().gen()
        charpoly = charpoly.map_coefficients(lambda c:c.simplify(error=error, force=True))
        isomorphic_extension = self.base_ring().extension(charpoly)
        return isomorphic_extension._base, isomorphic_extension._base_valuation

    def _simple_eisenstein_model(self):
        r"""
        Return a simple model of this extension generated by a uniformizing
        element.
        """
        if isinstance(self.base_ring(), Completion_base):
            return self._eisenstein_model()
        else:
            raise NotImplementedError
